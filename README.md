...
> **Provisional.** This plugin subclasses `papis.format.python._PythonStringFormatter`, which is private API.
...
> It is intended to be superseded by an upstream change (see `UPSTREAM.md`); if that lands, this repo will be archived with a pointer to the papis version that includes it.
...
Taking the first three words of a title is the only reliable way to disambiguate authors who publish several papers a year, but those words are frequently useless:
...
`uv tool install` is declarative — it rebuilds the tool environment from exactly the
requirements given on the command line. If your papis install already has extras
(`citeproc-py` for CSL output, say), repeat them in the same command or they will be
dropped:
...
The separators are backslash-escaped because `ref-word-separator =` (empty) is what collapses `Del Debbio` into `DelDebbio`, and that also eats unescaped underscores; `papis.bibtex.ref_cleanup` exempts `\_`.
